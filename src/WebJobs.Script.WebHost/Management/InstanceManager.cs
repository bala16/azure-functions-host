...
namespace Microsoft.Azure.WebJobs.Script.WebHost.Management
{
    public class InstanceManager : IInstanceManager
    {
            ...
            IEnvironment environment, ILogger<InstanceManager> logger, IMetricsLogger metricsLogger, IMeshServiceClient meshServiceClient, IRunFromPackageHandler runFromPackageHandler)
        {
            ...
            _runFromPackageHandler = runFromPackageHandler;
        ...
        // For PowerShell apps:
        // If Run-From-pkg and Azure Files are configured - content will be downloaded and unzipped into Azure Files.
        // In case of failures or if Run-From-pkg or Azure Files are not configured, local disk will be used instead.

        // For other apps:
        // If Run-From-Pkg is configured, content will be downloaded and unzipped/mounted into local disk.
        // If Run-From-Pkg is not configured, only then Azure Files will be mounted. Actual content will be deployed through other means to Azure Files.
        ...
        private async Task<bool> ApplyContextForPowerShell(HostAssignmentContext assignmentContext, RunFromPackageContext pkgContext)
        {
            try
            {
                ...
                {
                    _logger.LogDebug("Mounting AzureFilesContentShare");
                    await MountAzureFilesContentShare(assignmentContext);
                    return await _runFromPackageHandler.DeployToAzureFiles(assignmentContext, pkgContext);
                }
                else
                {
                    _logger.LogWarning(
                        $"PowerShell app with no {nameof(EnvironmentSettingNames.AzureFilesConnectionString)} or {nameof(EnvironmentSettingNames.AzureFilesContentShare)}");
                }
            ...
            {
                _logger.LogWarning(nameof(ApplyContextForPowerShell), e);
                return false;
            }
        }
